import { footerColumns, footerBottomLinks, socialLinks } from "../data/starbucksData";

// SVG social icons to avoid FA CDN dependency
const SocialIcon = ({ id }) => {
  const icons = {
...
export default function Footer() {
  return (
    <footer className="bg-white">
      {/* Column links */}
      <div className="border-b border-gray-200 py-10 px-6 md:px-16">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-8 max-w-screen-xl mx-auto">
          {footerColumns.map((col) => (
            <div key={col.heading}>
              <h4 className="text-base font-semibold text-gray-900 mb-3">{col.heading}</h4>
      ...
      {/* Social icons row */}
      <div className="border-b border-gray-200 px-6 md:px-16 py-5">
        <div className="flex gap-5 max-w-screen-xl mx-auto">
          {socialLinks.map((s) => (
            <a
              ...
              href={s.href}
              title={s.label}
              className="text-gray-700 hover:text-[#006242] hover:scale-125 transition-all duration-200"
            >
              <SocialIcon id={s.id} />
            </a>
          ))}
      ...
      {/* Bottom links */}
      <div className="px-6 md:px-16 py-5 max-w-screen-xl mx-auto">
        <ul className="flex flex-wrap gap-x-6 gap-y-2">
          {footerBottomLinks.map((link) => (
            <li key={link}>
              <a href="#" className="text-sm text-gray-600 hover:underline hover:text-gray-900 transition-colors">
                {link}
              </a>
            </li>
          ))}
      ...
      {/* Copyright */}
      <div className="px-6 md:px-16 pb-8 max-w-screen-xl mx-auto">
